...
def get_mol_embedding_func(feature):
    ...
    """
    if feature == 'gin':
        def embedding_func(smi): return mol_embedding(smi, device='cpu')
    elif feature == 'fp_4096':
        def embedding_func(smi): return fp_embedding(smi, _nBits=4096)
    elif feature == 'fp_2048':
        def embedding_func(smi): return fp_embedding(smi, _nBits=2048)
    elif feature == 'fp_1024':
        def embedding_func(smi): return fp_embedding(smi, _nBits=1024)
    elif feature == 'fp_512':
        def embedding_func(smi): return fp_embedding(smi, _nBits=512)
    elif feature == 'fp_256':
        def embedding_func(smi): return fp_embedding(smi, _nBits=256)
