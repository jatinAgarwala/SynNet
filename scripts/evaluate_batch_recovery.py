...
if __name__ == '__main__':
    ...
    generated_st_files = glob.glob(
        '../../results_mp/pis_fp/decode_result_train*.csv.gz')

    # lists in which to collect all the results
    recovered_molecules = pd.DataFrame({'query SMILES': [], 'decode SMILES': [
    ], 'similarity': []})  # molecules successfully recovered from query
    unrecovered_molecules = pd.DataFrame(
        {'query SMILES': [],
         'decode SMILES': [],
         'similarity': []})  # unsuccessfully recovered
    ...
    for generate_st_file in generated_st_files:
        ...
        unrecovered_molecules = pd.concat(
            [unrecovered_molecules,
             result_df[result_df['similarity'] != 1.0].dropna()])
        recovered_molecules = pd.concat(
            [recovered_molecules, result_df[result_df['similarity'] == 1.0].dropna()])
    ...
    # compute the following properties, using the TDC, for the succesfully
    # recovered molecules
    recovered_novelty_all = novelty(
        recovered_molecules['query SMILES'].tolist(),
        recovered_molecules['decode SMILES'].tolist())
    recovered_validity_decode_all = validity(
        recovered_molecules['decode SMILES'].tolist())
    recovered_uniqueness_decode_all = uniqueness(
        recovered_molecules['decode SMILES'].tolist())
    recovered_fcd_distance_all = fcd_distance(
        recovered_molecules['query SMILES'].tolist(),
        recovered_molecules['decode SMILES'].tolist())
    recovered_kl_divergence_all = kl_divergence(
        recovered_molecules['query SMILES'].tolist(),
        recovered_molecules['decode SMILES'].tolist())

    # compute the following properties, using the TDC, for the unrecovered
    # molecules
    unrecovered_novelty_all = novelty(
        unrecovered_molecules['query SMILES'].tolist(),
        unrecovered_molecules['decode SMILES'].tolist())
    unrecovered_validity_decode_all = validity(
        unrecovered_molecules['decode SMILES'].tolist())
    unrecovered_uniqueness_decode_all = uniqueness(
        unrecovered_molecules['decode SMILES'].tolist())
    unrecovered_fcd_distance_all = fcd_distance(
        unrecovered_molecules['query SMILES'].tolist(),
        unrecovered_molecules['decode SMILES'].tolist())
    unrecovered_kl_divergence_all = kl_divergence(
        unrecovered_molecules['query SMILES'].tolist(),
        unrecovered_molecules['decode SMILES'].tolist())

    print('N recovered, N unrecovered, N total (% recovered):', n_recovered,
          ',', n_unrecovered, ',', n_total, ', (', 100 * n_recovered / n_total, '%)')
    n_finished = n_recovered + n_unrecovered
    n_unfinished = n_total - n_finished
    print('N finished trees (%):', n_finished,
          '(', 100 * n_finished / n_total, '%)')
    print('N unfinished trees (NaN) (%):', n_unfinished,
          '(', 100 * n_unfinished / n_total, '%)')
    ...
    print(
        'Validity, decode molecules, recovered:',
        recovered_validity_decode_all)
    print('Validity, decode molecules, unrecovered:',
          unrecovered_validity_decode_all)

    print('Uniqueness, decode molecules, recovered:',
          recovered_uniqueness_decode_all)
    print('Uniqueness, decode molecules, unrecovered:',
          unrecovered_uniqueness_decode_all)
