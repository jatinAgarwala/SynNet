"""
This file contains the code to decode synthetic trees using beam search at the
first reactant sampling step (after the action network).
"""
import os
import pandas as pd
import numpy as np
...
from syn_net.utils.predict_beam_utils import synthetic_tree_decoder, load_modules_from_checkpoint
...
if __name__ == '__main__':
    ...
    param_path = (
        f"/home/rociomer/SynthNet/pre-trained-models/{args.rxn_template}"
    ...
    df.to_csv(
        f'{save_path}decode_result_{args.data}_robw_{str(args.beam_width)}.csv.gz',
        compression='gzip',
        index=False)

    synthetic_tree_set = SyntheticTreeSet(sts=trees)
    synthetic_tree_set.save(
        f'{save_path}decoded_st_robw_{str(args.beam_width)}_{args.data}.json.gz')
