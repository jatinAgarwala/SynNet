...
if __name__ == '__main__':
    ...
    result_train = pd.read_csv(
        '../results/decode_result_test_processed_property.csv.gz',
        compression='gzip')
    ...
    print(
        f"Novelty: {novelty(result_test_unrecover['query SMILES'].tolist(), result_test_unrecover['decode SMILES'].tolist())}")
    print(
        f"Validity: {validity(result_test_unrecover['decode SMILES'].tolist())}")
    print(
        f"Uniqueness: {uniqueness(result_test_unrecover['decode SMILES'].tolist())}")
    print(
        f"FCD: {fcd_distance(result_test_unrecover['query SMILES'].tolist(), result_test_unrecover['decode SMILES'].tolist())}")
    print(
        f"KL: {kl_divergence(result_test_unrecover['query SMILES'].tolist()[:10000], result_test_unrecover['decode SMILES'].tolist()[:10000])}")
