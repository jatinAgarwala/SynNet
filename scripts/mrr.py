...
if __name__ == '__main__':
    ...
    main_dir = '/pool001/whgao/data/synth_net/' + args.rxn_template + '_' + args.featurize + \
        '_' + str(args.radius) + '_' + str(args.nbits) + '_' + validation_option[12:] + '/'
    path_to_rt1 = '/home/whgao/scGen/synth_net/synth_net/params/' + \
        args.param_dir + '/' + 'rt1.ckpt'
    ...
    # for kw_metric_ in ['euclidean', 'manhattan', 'chebyshev',
    # 'cross_entropy', 'cosine']:
    ...
    for X, y in data_iter:
        ...
        y_hat = rt1_net(X)
        dist_true, ind_true = kdtree_fp_256.query(
            y.detach().cpu().numpy(), k=1)
        dist, ind = kdtree_fp_256.query(
            y_hat.detach().cpu().numpy(), k=bb_emb_fp_256.shape[0])
        ranks = ranks + [np.where(ind[i] == ind_true[i])[0][0]
