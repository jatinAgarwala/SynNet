...
def organize(synthetic_tree, d_mol=300, target_embedding='fp', radius=2, n_bits=4096,
    ...
    Args:
        synthetic_tree (SyntheticTree): The input synthetic tree to organize.
        ...
        n_bits (int, optional): Number of bits to use in the Morgan fingerprints.
            Defaults to 4096.
    ...
    if target_embedding == 'fp':
        target = mol_fp(synthetic_tree.root.smiles, radius, n_bits).tolist()
    elif target_embedding == 'gin':
        target = get_mol_embedding(synthetic_tree.root.smiles, model=model).tolist()
    else:
        raise ValueError('Target embedding only supports fp and gin.')

    most_recent_mol = None
    other_root_mol = None
    for i, action in enumerate(synthetic_tree.actions):

        most_recent_mol_embedding = mol_fp(
            most_recent_mol, radius, n_bits).tolist()
        other_root_mol_embedding = mol_fp(
            other_root_mol, radius, n_bits).tolist()
        state = most_recent_mol_embedding + other_root_mol_embedding + target

        if action == 3:
            step = [3] + [0] * d_mol + [-1] + [0] * d_mol + [0] * n_bits

        else:
            reaction = synthetic_tree.reactions[i]
            mol1 = reaction.child[0]
            if len(reaction.child) == 2:
                mol2 = reaction.child[1]
            else:
                mol2 = None
            ...
            if output_embedding == 'gin':
                step = ([action]
                        ...
                        + mol_fp(mol1, radius, n_bits).tolist())
            elif output_embedding == 'fp_4096':
                step = ([action]
        ...
        if action == 2:
            most_recent_mol = reaction.parent
            other_root_mol = None
...
def prep_data(main_dir, num_rxn, out_dim):
    ...
    for dataset in ['train', 'valid', 'test']:
        ...
        x_var = states
        y_var = steps[:, 0]
        sparse.save_npz(f'{main_dir}X_act_{dataset}.npz', x_var)
        sparse.save_npz(f'{main_dir}y_act_{dataset}.npz', y_var)
        ...
        x_var = sparse.hstack([states, steps[:, (2 * out_dim + 2):]])
        y_var = steps[:, out_dim + 1]
        sparse.save_npz(f'{main_dir}X_rxn_{dataset}.npz', x_var)
        sparse.save_npz(f'{main_dir}y_rxn_{dataset}.npz', y_var)
        ...
        y_var = steps[:, (out_dim + 2): (2 * out_dim + 2)]
        sparse.save_npz(f'{main_dir}X_rt2_{dataset}.npz', x_var)
        sparse.save_npz(f'{main_dir}y_rt2_{dataset}.npz', y_var)
        ...
        x_var = states
        y_var = steps[:, 1: (out_dim + 1)]
        sparse.save_npz(f'{main_dir}X_rt1_{dataset}.npz', x_var)
        sparse.save_npz(f'{main_dir}y_rt1_{dataset}.npz', y_var)
