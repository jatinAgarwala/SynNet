...
def can_react(state, rxns):
    ...
    mol1 = state.pop()
    mol2 = state.pop()
    reaction_mask = [int(rxn.run_reaction([mol1, mol2]) is not None)
                     for rxn in rxns]
...
def synthetic_tree_decoder(z_target,
    ...
    for i in range(max_step):
        ...
        z_state = set_embedding(
            z_target,
            state,
            nbits=n_bits,
            _mol_embedding=mol_fp)
        ...
        if rxn.num_reactant == 2:
            ...
            if act == 2:
                ...
                reactant2_net_input = torch.Tensor(np.concatenate(
                    [z_state, z_mol1, one_hot_encoder(rxn_id, num_rxns)], axis=1))
...
def synthetic_tree_decoder_rt1(z_target,
    ...
    for i in range(max_step):
        ...
        try:
            z_state = set_embedding(
                z_target, state, nbits=n_bits, _mol_embedding=mol_fp)
        except BaseException:
            ...
            if act == 2:
                ...
                if rxn_template == 'hb':
                    z_mol2 = reactant2_net(
                        torch.Tensor(
                            np.concatenate(
                                [z_state, z_mol1, one_hot_encoder(
                                    rxn_id, 91)],
                                axis=1)))
                elif rxn_template == 'pis':
                    z_mol2 = reactant2_net(
...
def synthetic_tree_decoder_multireactant(z_target,
    ...
    for i in range(beam_width):
        tree, act = synthetic_tree_decoder_rt1(
            ...
            bb_dict=bb_dict, reaction_templates=reaction_templates,
            mol_embedder=mol_embedder, action_net=action_net,
            reactant1_net=reactant1_net, rxn_net=rxn_net,
            reactant2_net=reactant2_net, bb_emb=bb_emb,
            rxn_template=rxn_template, n_bits=n_bits, max_step=max_step,
            rt1_index=i)

        similarities_ = np.array(
            tanimoto_similarity(
                z_target, [
                    node.smiles for node in tree.chemicals]))
